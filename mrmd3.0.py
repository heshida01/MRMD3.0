...
def parse_args():
    ...
    parser.add_argument("-m", "--mode", dest='m', type=str, default='0',
                        help="0:normal , 1:fast")
    parser.add_argument("-j", "--n_jobs", dest='j', type=str, default='1',
                        help="The number of jobs to run in parallel for MRMD distance")
...
class Dim_Rd(object):
    ...
    def run(self, inputfile):
        ...
        features, features_sorted = feature_rank(file, self.logger,args.r,args.m,args.j)
        ...
        if args.m == "0": # 标准
            ...
            self.Dim_reduction_cluster(features, features_sorted, metrics_file, csvfile)
            return features_sorted
...
if __name__ == '__main__':
    ...
    def lineplot_(in_file):
        ...
        plt.title("The metrics change according to the feature ranking result")
        plt.xlabel('features num', fontsize=18)
    ...
    def violin(in_file,topn = 15):
        ...
        df = pd.read_csv(in_file).iloc[:,:topn+1]
    ...
    def heatmap_(in_file, topn = 15):
        plt.clf()
        scaler = StandardScaler()
        df = pd.read_csv(in_file).iloc[:,:topn+1]  # + label
        sns.set_theme(color_codes=True)
